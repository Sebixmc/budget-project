...
- [ ] **Inline merchant-rule creation (post-upload triage + save-as-rule prompt)** — after a CSV import, a panel groups still-uncategorized transactions by merchant so one category pick creates a rule and fixes the whole group; on Transactions, an inline category edit offers "save as rule · fixes N more". Spec: [`specs/rule-triage-flows.md`](specs/rule-triage-flows.md). `[IN PROGRESS — 2026-08-04: implemented (triage panel, save-as-rule prompt, lib/merchant.ts, 31 Vitest green, web CI green); remaining: in-browser Test-plan pass — the dev container's firewall blocks Supabase, see spec Amendment]`
- [ ] **Budget builder — the paycheck cascade** — rebuild the Budget page as a top-to-bottom flow: gross salary → itemized tax estimate → prominent ÷12 pivot → savings goals ("pay yourself first") → category envelopes with historical-average hints and a sticky left-to-allocate bar; Sankey uses after-tax income + goal branches. Two new tables (`budget_profile`, `savings_goals`) with RLS in migration 0003. Spec: [`specs/budget-builder.md`](specs/budget-builder.md). `[IN PROGRESS — 2026-08-04: implementation started by devcontainer agent]`
...
- [ ] **Custom categories** — let the user create their own categories (not just rename/merge/delete the built-ins) from Settings → Category management. New `user_categories` table (user_id + RLS); the category list served to every dropdown becomes the dynamic union of built-ins + custom + in-use. Spec: [`specs/custom-categories.md`](specs/custom-categories.md). `[IN PROGRESS — 2026-08-04: implemented (migration 0006_user_categories + RLS; validateCreate in lib/categories.ts + tests; getSelectableCategories() in lib/data/categories.ts feeding Transactions/Rules/Budget dropdowns; Settings "Add category" UI; delete AND rename/merge both carry user_categories — a code-review caught that rename originally orphaned custom rows). Web CI green: lint, typecheck, 86 Vitest, next build compiles + types validate. Remaining: in-browser Supabase pass — container has no Supabase env (next build prerender of /settings needs it), same blocker as the rewrite items above]`
- [ ] **Settings page overhaul** — turn `/settings` from account-only CRUD into the app's preferences hub: consolidated Accounts section + Appearance (light/dark/system), Category management (rename/merge/delete), Profile & security (email + change password), Budget preferences (income + default landing page), and Data & privacy (export all data, danger-zone delete-all). Adds a `user_settings` table. Spec: [`specs/settings-page-overhaul.md`](specs/settings-page-overhaul.md). `[IN PROGRESS — 2026-08-04: implemented (all six sections, migration 0004_user_settings, rename/merge/delete + income + default-page + export + delete-all; login honors default page). Web CI green (lint, typecheck, 61 Vitest, next build); dev server boots + auth guard verified. Remaining: in-browser Supabase pass — container firewall blocks Supabase, same as the rewrite items above]`
